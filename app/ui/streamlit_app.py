...
def main():
    ...
    wind_upload = st.sidebar.file_uploader("Wind-Datei (03379_*stadt.txt)", type=["txt", "csv"])
    cloud_upload = st.sidebar.file_uploader("Cloud-Datei (produkt_n_stunde_*.txt)", type=["txt", "csv"])
    year = st.sidebar.selectbox("Jahr", list(range(2000, 2026)), index=9)
    lat = st.sidebar.number_input("Breite (lat)", value=config.DEFAULT_LAT, format="%0.4f")
    lon = st.sidebar.number_input("Länge (lon)", value=config.DEFAULT_LON, format="%0.4f")
    ...
    wind_path_txt = st.sidebar.text_input("Wind-Pfad", value="data/raw/wind/03379_Muenchen_stadt.txt")
    cloud_path_txt = st.sidebar.text_input(
        "Cloud-Pfad", value="data/raw/cloudiness/produkt_n_stunde_19790101_20241231_03379.txt"
    )
    ...
    if st.sidebar.button("Pipeline starten"):
        wind_path = _persist_upload(wind_upload, suffix=".txt") if wind_upload else Path(wind_path_txt)
        cloud_path = _persist_upload(cloud_upload, suffix=".txt") if cloud_upload else Path(cloud_path_txt)
        ...
        try:
            ...
            with open(paths["ausbreitung"], "rb") as f:
                st.download_button("Ausbreitung CSV laden", data=f, file_name=paths["ausbreitung"].name)
            with open(paths["akterm"], "rb") as f:
                st.download_button("AKTERM laden", data=f, file_name=paths["akterm"].name)
            with open(paths["assumptions"], "rb") as f:
                st.download_button("assumptions.json laden", data=f, file_name="assumptions.json")
